...
class enhance_mapping_id(object):
	...
	def process (self, parameters={}, data={} ):
		...
		if 'mappings' in parameters:
			parameters['id'] = mapping( parameters['id'], parameters['mappings'] )
...
def mapping( value, mappings={} ):
	...
	for map_from, map_to in mappings.items():
		...
		if value.startswith(map_from):
			...
			# if from string longer (deeper path), its the better matching
	...
	# if there is a match, replace value with mapping
